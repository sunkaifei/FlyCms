...
public abstract class ScaleImageUtils {
     ...
     * <b>function:</b> 将指定的targetFile图片文件的宽度、高度大于指定width、height的图片缩小，并保存在savePath目录
     * @author sunkaifei
     * @createDate 2015-12-25 下午06:27:02
     * @param width 指定宽度
     * @param height 指定的高度
     * @param savePath 保存目录
     * @return 图片保存路径、名称
     * @throws IOException
     */
    public static String forcedResize(int width, int height, String savePath, File targetFile) throws IOException {
        image = ImageIO.read(targetFile);
        int[] size = getSize(width, image);
        int[] size1 = getSizeByHeight(height, image);
        return resize(size[0], size1[1], savePath, image);
    }
